...
export const collabStore = {
  getCollabConf: (): StoredCollabConfig | null => {
    const raw = localStorage.getItem('dsheet_collabConfig');
    return raw ? JSON.parse(raw) : null;
  },
  setCollabConf: (config: StoredCollabConfig) => {
    localStorage.setItem('dsheet_collabConfig', JSON.stringify(config));
  },
  clearCollabConf: () => {
    localStorage.removeItem('dsheet_collabConfig');
  },
};
