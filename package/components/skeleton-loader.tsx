...
const useGridDimensions = (
  ...
  const TOOLBAR_HEIGHT = isReadOnly ? 0 : 48; // approximate toolbar height
  ...
  const availableHeight = height - HEADER_HEIGHT - TOOLBAR_HEIGHT;
...
export const SkeletonToolbar = () => {
  ...
  const sections = [
    { items: 1, gap: 0 }, // First group of 4 items
      ...
      {sections.map((section, sectionIndex) => (
        <div key={sectionIndex} className="flex items-center">
          ...
          {sectionIndex < sections.length - 1 && (
            <div className="mx-3 h-5 w-px bg-gray-200" /> // Divider
          )}
        </div>
      ))}
    </div>
...
export const SkeletonGrid = ({
  ...
  const containerRef = useRef<HTMLDivElement>(null);
  const { cols, rows } = useGridDimensions(containerRef, isReadOnly);
...
const SkeletonLoader = ({
  ...
  <div className="w-full h-full flex flex-col">
    ...
    <div className="flex-1 overflow-hidden">
      <SkeletonGrid isReadOnly={isReadOnly} />
    </div>
  </div>
);
