...
export const useRefreshDenomination = ({
  ...
  const cryptoPriceRef = useRef<{ bitcoin: Record<string, number>, ethereum: Record<string, number>, solana: Record<string, number> } | null>(null);
  ...
  const refreshDenomination = async () => {
    ...
    for (let i = 0; i < cellData?.length; i++) {
      ...
      cell.v.m = value.replace(/\d+(\.\d+)?/, (Number(cell.v?.v) / price).toFixed(decemialCount).toString
        ());
