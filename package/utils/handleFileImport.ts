...
export const handleFileUploadUtil = async (event: React.ChangeEvent<HTMLInputElement>): Promise<any> => {
    const input = event.target;
    if (!input.files?.length) {
        return;
    }
    const file = input.files[0];
    let dropdownInfo: { [key: string]: { type: string; formulae: string[] } } | null = null;
            ...
            if (worksheet) {
                ...
                worksheet.eachRow((row, rowIndex) => {
                    row.eachCell((cell, colIndex) => {
                        ...
                        if (dataValidation && dataValidation.type === 'list' && dataValidation.formulae) {
